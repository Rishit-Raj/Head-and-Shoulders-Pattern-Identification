...
def check_hs_pattern(extrema_indices: List[int], data: np.array, i:int, early_find: bool = False) -> HSPattern:
    ...
    if data[l_shoulder] < 0.9*r_midpoint  or data[r_shoulder] < 0.9*l_midpoint:
        return None
...
def TestPattern(data, hs_patterns, ihs_patterns, hs_df, ihs_df):
    ...
    for i in range(len(hs_df)):
        initial_capitalHS = initial_capitalHS * (1 + hs_df.loc[i, 'hold_return'])
        if hs_df.loc[i, 'hold_return'] >= 0:
            positiveHSreturns.append(100*hs_df.loc[i, 'hold_return'])
        else:
            negativeHSreturns.append(100*hs_df.loc[i, 'hold_return'])

        capitalListHS.append(initial_capitalHS)
...
if __name__ == '__main__':
    ...
    if len(hs_patterns) > 0:
        print('These are the dates the head-and-shoulders patterns appeared in the last 10 years')
